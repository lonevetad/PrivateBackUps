...
import lol.mtAvl.MapTreeAVLFull;
import lol.mtAvl.MapTreeAVLLightweight;
import lol.mtAvl.MapTreeAVLMinIter;
import lol.mtAvl.MapTreeAVLQueuable;
...
public interface MapTreeAVL<K, V> extends Serializable, SortedMap<K, V>, Function<K, V>, Iterable<Entry<K, V>> {
	...
	// Imported from old MapTreeAVLFactory
	 ...
	 */
	public static enum Optimizations {
		/**
	...
	public static <K, V> MapTreeAVL<K, V> newMap(Optimizations optimization, BehaviourOnKeyCollision b,
		...
		m = null;
		switch (optimization) {
		case FullButHeavyNodes:
			m = new MapTreeAVLFull<K, V>(b, comp);
			break;
		case Lightweight:
			m = new MapTreeAVLLightweight<K, V>(b, comp);
			break;
		case MinMaxIndexIteration:
			m = new MapTreeAVLMinIter<K, V>(b, comp);
			break;
		case ToQueueFIFOIterating:
			m = new MapTreeAVLQueuable<K, V>(b, comp);
			break;
		default:
			throw new IllegalArgumentException("Weird optimization option: " + optimization);
		}
		return m;
	}
