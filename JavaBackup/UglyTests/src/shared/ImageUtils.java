...
public class ImageUtils {
	...
	public static final boolean writeImages(BufferedImage[] a, String pathFolder, String name, String extensionNoDot,
		...
		if (ret) {
			...
			if (extensionNoDot.length() == 0) {
				extensionNoDot = ImageUtils.extensionImageFile[0];
			} else {
				notFound = true;
				toLowCase = extensionNoDot.toLowerCase();
				for (i = 0; (i < ImageUtils.extensionImageFile.length && notFound); i++) {
					notFound = !(toLowCase.contains(ImageUtils.extensionImageFile[i]));
				}
				if (notFound) {
					extensionNoDot = ImageUtils.extensionImageFile[0];
				} else {
	...
	public static BufferedImage readImage(String path, String nameNoExtension) {
		...
		if (path != null && nameNoExtension != null && nameNoExtension.length() > 0) {
			...
			while (ret == null && ++i < ImageUtils.extensionImageFile.length) {
				pathCompleto = pathParziale + '.' + ImageUtils.extensionImageFile[i];
