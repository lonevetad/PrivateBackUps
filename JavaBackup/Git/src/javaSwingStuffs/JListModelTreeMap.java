...
public class JListModelTreeMap<K, E> extends AbstractListModel<E> implements Set<E> {
	...
	public static <K, E> JListModelTreeMap<K, E> newInstance(Comparator<K> comparatorKeys,
			Function<E, K> keyExtractor) {
		return new JListModelTreeMap<K, E>(comparatorKeys, keyExtractor);
	}

	public static <Same> JListModelTreeMap<Same, Same> newInstance(Comparator<Same> comparatorKeys) {
	...
	protected JListModelTreeMap(Comparator<K> comparatorKeys, Function<E, K> keyExtractor) {
		this(keyExtractor);
...
//		RedBlackTree<K, E>.RBTIterator 
		...
		iter = (delegate.keySet()).iterator();
		while (iter.hasNext() && //
				(notFound = (elem == null ? (iter.next() != null)
