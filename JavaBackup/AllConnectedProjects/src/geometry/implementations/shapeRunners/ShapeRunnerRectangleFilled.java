...
//			runRectangleNonRotated(shape, action);
	...
	public static void runRectangleNonRotated(PointConsumer action, int xCentre, int yCentre, int width, int height) {
		...
		if (width <= 0 || height <= 0 || action == null)
			return;
		p = new Point(yCentre - (height >> 1), x = xCentre - (width >> 1));
		while (height-- > 0 && action.canContinue()) {
			ShapeRunnerLine.runHorizontalSpan(action, p, width);
			p.x = x;
			p.y++;
		}
	}
	 ...
	 */
	public static void runRectangleRotated(PointConsumer action, Point2D[] corners) {
		if (PointConsumer.FORCE_EARLY_STOPPING)
			runRectangleRotated_EarlyStopped(action, corners);
		else
			runRectangleRotated_NonStopping(action, corners);
	}

	public static void runRectangleRotated_NonStopping(PointConsumer action, Point2D[] corners) {
		int x, y, lasty, y2, y3;
...
//		c = new Point2D[] { corners[0], corners[1], corners[2], corners[3] }
		...
		while (++y < lasty) {
			...
			ShapeRunnerLine.runHorizontalSpan(action, p, //
					((int) Math.round((y - qr) / slopeRight)) - x);
		}
	 ...
	 * BEWARE: it modify the original array !
	 */
	public static void runRectangleRotated_EarlyStopped(PointConsumer action, Point2D[] corners) {
		int x, y, lasty, y2, y3;
		double slopeLeft, slopeRight, ql, qr; /* slope12, slope13, slope24, slope34, */
		Point2D p1, p2, p3, p4; // , c[]; // == top, left, right, bottom
		Point p;
