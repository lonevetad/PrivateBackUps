...
import common.EnumElementTileImageFactory;
 ...
 * This interface is used to mark an object as an element of a <i>enumeration</i> of objects able to
 * create a {@link GameObjectInMap} and its View's part: {@link TileImage}, which contains the
 * images and animations. These instances are combined together to create a
 * {@link GameObjectInMapView} instance, used by graphic environment like {@link MapGameView}.<br>
 * 
...
public interface AbstractEnumElementGOTI extends Serializable {
	...
	/**
	 * Create a <b>new</b> {@link TileImage} and scale its image thanks to the informations provided
	 * with {@link MainController}, in particular
	 * {@link MainController#getMainGenericGUI()}.{@link MainGUI#getPixelEachMicropixel()}..<br>
	 * If there's a jet stored TileImage in cache, then a clone (a <b>new</b> one) of it is
	 * returned.<br>
	 * Otherwise, a <b>new</b> TileImage is created.<br>
	 * WARNING: If resizing the images is needed, resize all of TileImage got previously and stored
	 * in some way, don't use the returned one.
	 */
	 ...
	 * {@link #newGameObjectInMap(MainController, String, ShapeSpecification, boolean)} and passing
	...
	public default GameObjectInMap newGameObjectInMap(MainController main, String name, ShapeSpecification ss,
		...
		if (main == null || name == null || ss == null || goimf == null) return null;
		goim = goimf.newGameObjectInMap(main, name, ss, isNotSolid);
		if (goim != null) {
			if (getID() != null) goim.setIdEnumElement(getID());
			if (name != null) goim.setName(name);
			if (ss != null) goim.setShapeSpecification(ss);
			if (main != null) goim.setMain(main);
		}
		return goim;
	}

	/**
	 * Get all parts (the Model {@link GameObjectInMap} and the View {@link TileImage}) and pack
	 * them in a single wrapper: {@link GameObjectInMapView}.
	 */
	public default GameObjectInMapView newGameObjectInMapView(MainController main) {
		...
		if (main != null) {
			...
			if (goim != null) {
				...
				if (ti != null) goimv = new GameObjectInMapView(main.getMainGenericGUI(), goim, ti);
			}
		}
		return goimv;
	}
	...
	/**
	 * If and only if there's a image(s) stored (obtained through {@link #getTileImageCached()}),
	 * then that {@link TileImage} will be scaled depending of its {@link ShapeSpecification} (that
	 * shape is expressed id {@link AbstractMatrixObjectLocationManager}'s <i>micropixel</i> and
	 * works ONLY with {@link AbstractObjectRectangleBoxed} on this default method implementation)
	 * and the given parameter "pixelEachMicropixel": the amount of real graphical pixels
	 * represented by a single <i>micropixel</i>.<br>
	 * The stored image(s) will be scaled ready to be drawn on the View (screen).
	 * <p>
	 * WARNINGS: scaling the stored {@link TileImage} WON'T affect any other TileImage created
	 * through {@link #newGameObjectInMap(MainController)} and its overload(s), but only the one
	 * stored in {@link #getTileImageCached()}. Store that created TileImages in some way and resize
	 * them manually, one per one, calling {@link TileImage#scaleImages(int, int)}.
	 * 
	 * @param pixelEachMicropixel
	 *            amount of real graphical pixels represented by a single
	 *            {@link AbstractMatrixObjectLocationManager}'s <i>micropixel</i>. This parameter
	 *            should be setted with the value returned by
