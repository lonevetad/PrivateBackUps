...
public abstract class ObjectWithID
	...
	public static final MyComparator<ObjectWithID> COMPARATOR_OWID = (o1, o2) -> {
		if (o1 == o2) return 0;
		if (o1 == null) return -1;
		if (o2 == null) return 1;
	...
	/**
	 * Questo ID puo' essere usato nell'algoritmo di raccolta di oggetti in una data area : per
	 * verificare che un oggetto non sia gia' presente, si ricerca in un RedBlackTree usando questo
	 * campo come chiave. Così inserzione e ricerca sono di ordine O(log(n)).
	 */
	...
	/**
	 * If <code>true</code>, this object is part of the background and is "walkable", like a tile of
	 * grass, sand or street.<br>
	 * If <code>false</code>, then it's something physical, like a tree, a stone, a creature, a NPC,
	...
	public boolean canAddMeToMolm(AbstractMatrixObjectLocationManager molm) {
		if (molm == null) return false;
		return false;
	}

	public String addMeToMolm(AbstractMatrixObjectLocationManager molm) {
		if (molm == null) return "ERROR: on ObjectWithID.addMeToMolm, molm is null";
		return null;
	}

	public String removeMeToMolm(AbstractMatrixObjectLocationManager molm) {
		if (molm == null) return "ERROR: on ObjectWithID.removeMeToMolm, molm is null";
		return null;
	}

	/*
	 * public boolean canAddMeToMolm(MatrixObjectLocationManager molm, int xx, int yy) { if (molm ==
	 * null) return false; return false; } public boolean addMeToMolm(MatrixObjectLocationManager
	 * molm, int xx, int yy) { if (molm == null) return false; return false; } public boolean
	 * removeMeToMolm(MatrixObjectLocationManager molm, int xx, int yy) { if (molm == null) return
	 * false; return false; }
	 */
	...
	public boolean reloadState(Memento me) {
		...
		if (me != null && me instanceof MementoOWID) {
			...
			if (log == null) log = LoggerMessages.LOGGER_DEFAULT;
			return true;
		}
		return false;
	}
