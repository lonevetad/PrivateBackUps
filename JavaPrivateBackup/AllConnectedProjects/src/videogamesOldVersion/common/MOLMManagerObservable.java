...
public class MOLMManagerObservable extends MOLMManager implements ObservableMolm {
	...
	private static final long serialVersionUID = -4502984108741601L;

	public MOLMManagerObservable() {
		super();
	}

	public MOLMManagerObservable(MementoMOLMManager mmm, FullReloadEnvironment fre) {
		super(mmm, fre);

	}

	List<ObserverMolm> observersMolm;

	protected void commonCostructor() {
		observersMolm = new ArrayList<>(4);
	}
	...
	@Override
	public String doOnMOLM(ObjectWithID owid, DoSomethingWithMolmShapespecOwid doSome,
			AbstractMatrixObjectLocationManager preferredMolm) {
		String error;
		error = super.doOnMOLM(owid, doSome, preferredMolm);
		if (error != null) notifyMolmObservers(MolmEvent.CustomOperation, preferredMolm, this);
		return error;
	}
